...
lookup_table_for_rnaids <- NULL #'../azorius/cta2.0_probelookup.txt'
...
experiment_name <- 'hPDAC_WTA_20210222T2101' #' MK017_CPA_300_thresh10_keeplocals'
annotations_file <- 'data/second_acquisition/WTA/hPDAC_WTA_Annotations-hPDAC_WTA_20210222T2101_SampleIndexList_updated2.xlsx' #' ../MK017_CPA_300um_samplesheet.xlsx' # Sample Annotation File
annotations_sheet_name <- "Template" # Sample Annotation Sheet

# DCC Files input
# Set both to NULL if just using dedup & summary files
dcc_directory <- "data/second_acquisition/WTA/SN0216255/dcc" # NULL
output_prefix <- "output/hPDAC_WTA_20210222T2101" # NULL #prefix for output summary and dedup files
n_processors <- 10

# Summary and dedup files, optional
# Will get overwritten if using dcc folder
dedup_file <- NULL #'../MK017_dedup.txt' # Deduplicated Counts table
processing_summary_file <- NULL #'../MK017_summary.txt' # Summary Table
...
# Set to NULL if using PKC file(s)
targetGroups_file <- NULL #'CTA2.0_probe_groups_short.txt'
...
samp_nm_cols <- c("Sample_ID") #c("Sample_ID")
...
annot_cols <- c('slide_name','scan_name','roi_number','x_coordinate','y_coordinate','segment','aoi_area','roi_mask_area','nuclei_count','immune_proportion','CAF_proportion','epithelial_proportion','not_collected') #c('cell_line','roi_diam','rep','roi_num')

# Figure Generation Options
skipgpdf <- FALSE #TRUE # Set to true to skip making gene pdf

no.figures <- FALSE #TRUE # Set to true to skip making all figures
...
minLOQ = 0 # set a minimum LOQ, any calculated LOQ below this value will be reset to the minimum
...
housekeepers <- c() #c('C1orf43','GPI','OAZ1','POLR2A','PSMB2','RAB7A','SDHA','SNRPD3','TBC1D10B','TPM4','TUBB','UBB')
...
rm(probe_matrix, removed_probes, dedup_counts) #Payman -- moved down from above since dedup_counts was erased too early.
